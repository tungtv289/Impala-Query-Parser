...
//        System.out.println(System.getProperty("java.library.path"));
        ...
        try {
            ...
            if (node instanceof SelectStmt) {
                for (TableRef tblRef : ((SelectStmt) node).getTableRefs()) {
                    List<String> tmp = getFullTableName(tblRef);
                    rs.addAll(tmp);
                }
                if (((SelectStmt) node).hasWithClause()) {
                    for(View view : ((SelectStmt) node).getWithClause().getViews()) {
                        for (TableRef tblRef : ((SelectStmt) view.getQueryStmt()).getTableRefs()) {
                            List<String> tmp = getFullTableName(tblRef);
                            rs.addAll(tmp);
                        }
                    }
                ...
                System.out.println("ResetMetadataStmt");
            } else if (node instanceof ComputeStatsStmt) {
                System.out.println("ComputeStatsStmt");
            } else if (node instanceof DescribeTableStmt) {
                System.out.println("DescribeTableStmt");
            } else if (node instanceof AlterTableOrViewRenameStmt) {
                System.out.println("AlterTableOrViewRenameStmt");
            } else if (node instanceof AlterTableAddReplaceColsStmt) {
                System.out.println("AlterTableAddReplaceColsStmt");
            } else if (node instanceof AlterTableAlterColStmt) {
                System.out.println("AlterTableAlterColStmt");
    ...
    public static List<String> getFullTableName(TableRef tblRef) {
        List<String> rs = new ArrayList<>();
        if (tblRef instanceof InlineViewRef) {
            InlineViewRef inlineViewRef = (InlineViewRef) tblRef;
            ParseNode viewStmt = inlineViewRef.getViewStmt();
            if (viewStmt instanceof SelectStmt) {
                for (TableRef tblRef1 : ((SelectStmt) viewStmt).getTableRefs()) {
                    rs.addAll(getFullTableName(tblRef1));
                }
            }
        } else {
